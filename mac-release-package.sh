...
PACKAGE=SNACK-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=SNACK-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} SnackCoin.exe SnackCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe SnackCoin.exe
cp installer/lib/JavaExe.exe SnackCoinservice.exe
cp -a ${FILES} nxt
